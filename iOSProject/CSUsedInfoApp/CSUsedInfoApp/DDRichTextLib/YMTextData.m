...
- (void)calculateShowImageHeight{
  ...
  if (self.showImageArray.count == 0) {
    ...
    self.showImageHeight = ShowImage_H + 10;// 在首页的cell中最多只显示一行的图片信息
...
- (void)matchString:(NSString *)dataSourceString fromView:(BOOL) isYMOrNot{
  ...
  if (isYMOrNot == YES) {
    ...
    //匹配姓名
    NSMutableArray *name = [ILRegularExpressionManager matchName:dataSourceString];
    for (int i = 0; i < name.count; i ++) {
      
      [totalArr addObject:[name objectAtIndex:i]];
    }
    ...
    for (int i = 0; i < name.count; i ++) {
      ...
      [self.attributedDataWF addObject:[name objectAtIndex:i]];
    }
