...
- (void)setYMViewWith:(YMTextData *)ymData{
  ...
  //cell的图片加载部分
  //在首页的cell展示里，最多只显示三张图片
  for (int i = 0; i < MIN(3, [_imageArray count]); i++) {
  ...
  [_imageArray removeAllObjects];//release _imageArray
  
  for (int  i = 0; i < MIN(3, [ymData.showImageArray count]); i++) {
